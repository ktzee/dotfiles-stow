...
sed -i "/hwmon-path/c\ \ \ \ \"hwmon-path\": \"$HWMON_PATH\"," $SDIR/config-sway && sed -i "/hwmon-path/c\ \ \ \ \"hwmon-path\": \"$HWMON_PATH\"," $SDIR/config-sway

#waybar -c "$SDIR"/config1 -s "$SDIR"/style1.css &
waybar -c "$SDIR"/config-sway -s "$SDIR"/style.css &
